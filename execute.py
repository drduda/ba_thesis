...
def simulate_input_data(pupil_param_list = PUPIL_PARAM_LIST, eye_center=EYE_CENTER):
    """
...
def reprojection(tracked_ellipses, visualize=True):
    """
    ...
    actual_center = EYE_CENTER

    distance = float(np.linalg.norm(tracked_eye_center - actual_center))

    if visualize:
        plt.plot(actual_center[0], actual_center[1], marker='o', label='Actual center')
